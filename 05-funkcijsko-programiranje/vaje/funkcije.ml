...
let rec map_tlrec f xs =
  let rec map' f xs acc =
    match xs with
    | [] -> reverse acc (*sicer je napačen vrstni red*)
...
let rec mapi f list = 
  let rec mapi' f list mapi_list index = (*mapi_list je acc*)
    match list with
    | [] -> reverse mapi_list
    | x :: xs -> 
...
let rec zip  xs ys = 
  ...
  match xs, ys with
  | [], [] -> reverse acc
  | _ , [] | [], _ -> failwith "Different lengths of input lists." (* _ pomeni karkoli*)
